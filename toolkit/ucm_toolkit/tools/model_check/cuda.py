...
def main() -> int:
    check_config()
    # A worker re-enters this function with UCM_MODEL_CHECK_RANK set and
    # executes the native path. The controller does not initialize a device.
    ...
    log(f"CUDA_VISIBLE_DEVICES={devices}, device={active_device}")
    vllm_config = None
    fixture = None
    worker = None
    dispatch = None
    try:
        ...
        if fixture.world_size > 1:
            # Until connector metadata has a rank-aware serialization protocol,
            # TP mode is deliberately a layout-only check.  Running one
            # Scheduler per rank would produce unsafe duplicate dump/load work.
            dist_barrier()
